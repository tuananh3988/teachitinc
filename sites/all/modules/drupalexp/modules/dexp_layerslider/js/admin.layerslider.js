  ...
  function addLayerTab(layerIndex) {
    ...
    switch ($slides[$currentSlide].layers[layerIndex].type) {
      ...
      case 'video':
        ...
        content = '<div style="width:'+$slides[$currentSlide].layers[layerIndex].video_width+'px;height:'+$slides[$currentSlide].layers[layerIndex].video_height+'px;"></div>';
        break;
      case 'text':
    ...
    var right = null;
    if(left=='left') left = 0;
    if(left=='right') left = $settings.startwidth-newLayerDesign.width();
    if(left=='center') left = ($settings.startwidth-newLayerDesign.width())/2;
    newLayerDesign.css({
